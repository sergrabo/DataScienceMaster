...
CREATE TABLE  Localidades (
  idlocalidad char (3) NOT NULL , --id
  nombre  char(50) NOT NULL,  -- name
  codigopostal char(5),  --postal code
  PRIMARY KEY  (idlocalidad)
);

-- Clients
create table Clientes (
  idcliente integer NOT NULL , --id
  nif  char(12) NOT NULL, -- id card
  nombre char(50) not null, -- name
  apellido1 char(50) not null,-- surname 1
  apellido2 char(50) , -- surname 2
  telefono char(12) not null, -- telephone
  calle char(100) not null, -- street
  numero char(4) not null, --street number
  letra char(1), -- letter
  piso char (3), -- floor
  puerta char (2), -- door
  observaciones varchar(200), -- observations
  idlocalidad char(3) not  null, -- locality
...
create table Personal (
  idpersonal integer NOT NULL , -- id
  nif  char(12) NOT NULL UNIQUE, -- id card
  nombre char(50) not null, --name
  apellido1 char(50) not null, -- surname 1
  apellido2 char(50) , -- surname 2
  telefono char(12), -- telephone
  email char(50) check (email like '%@%'), -- email
  calle char(100) not null, -- street
  numero char(4) not null, -- street number
  piso char (3), -- floor
  puerta char (2), -- door
  idlocalidad char(3) not  null, -- locality
  fechaAlta date not null default '2018-11-21', -- registration date
  fechaBaja date, -- withdrawal date
...
create table Estados (
	idestado char (3) not null, -- id
	descripcion char (20) not null, -- description
...
create table Ingredientes (
	idingrediente char (3) not null, --id
	descripcion char (20) not null, -- description
...
-- Pizzas
create table Pizzas (
	idpizza integer not null, --id
	idbase char (3) not null, -- base
	importeBase numeric(5,2) not null, -- import
	PRIMARY KEY (idpizza),
...
-- Pizza ingredients (Many to many relationship)
create table IngredienteDePizza (
	idpizza integer not null, -- pizza
	idingrediente char (3) not null, -- ingredient
            cantidad int not null default 1, -- quantity
...
create table Pedidos (
	idpedido integer not null, -- id
	iva integer not null check (iva >=0 and iva<=100), -- vat
	fechaHoraPedido datetime not null, -- date and time of the order
	fechaHoraServicio datetime null, -- date and time of the deliver
	observaciones varchar (100) null, -- observations
	idestado char(3) not null, -- state
	idcliente integer not null, -- client
	idpersonal integer not null, -- staff
...
create table LineasPedidos (
	idpedido integer not null, -- order
	idlinea integer not null, -- line number
            idpizza integer not null, -- pizza
            precioUnidad money not null, -- unitary prize
            cantidad integer not null default 1, -- quantity
            descuento decimal(4,2) not null default 0, -- discount
