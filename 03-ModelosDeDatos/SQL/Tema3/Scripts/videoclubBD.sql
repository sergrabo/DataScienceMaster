...
-- 
-- M�ster en Data Science -- M1967 - Modelos de Datos y Sistemas de Informaci�n 2021-2022
-- DIego Garc�a Saiz
-----------------------------------------------------------------------------------------------------------
 
 -- Genre table
CREATE TABLE generos (
  id_genero INTEGER NOT NULL primary key, -- genre id
  desc_genero VARCHAR(100) NOT NULL); -- genre description
...
create table tarifas (
   codigo char(2) not null primary key, -- prize code
   descripcion char (20) not null, -- description
   importeDia decimal(4,2) not null default 0 -- daily prize
);
...
CREATE TABLE peliculas (
  id_pelicula INTEGER NOT NULL primary key, -- film id
  nombre_pelicula VARCHAR(200) NOT NULL, -- film name
  fecha_estreno DATE NOT NULL, -- date of release
  reparto VARCHAR(500) NOT NULL, -- actors
  id_genero INTEGER, -- genre if
  director VARCHAR(100) , -- director
  sinopsis VARCHAR(1000), -- synopsis
  refTarifa char(2) not null,	-- prize reference (foreing key)
...
-- film copis
CREATE TABLE copias (
     id_pelicula INTEGER NOT NULL, -- film id
     nro_copia INTEGER NOT NULL, -- copy number
     fechaAlta date not null, -- date of registration
     fechaBaja date null, -- date of withdrawal
...
CREATE TABLE Localidades (
      id_Localidad char (3)not null primary key, -- locality id
      Nombre char (30) not null --name
);
...
-- member table
CREATE TABLE  socios  (
     nro_socio  integer NOT NULL primary key, -- number or code of the member
     Nif  char(10) NOT NULL, -- id card
     Nombre char (100) NOT NULL, -- name
     Apellido1 char (100) NOT NULL, -- surname 1
     Apellido2 char (100) NOT NULL, -- surname 2
     Dirección char (100) NOT NULL, -- postal address
     id_Localidad char (3) NOT NULL, -- locality id
     Telefono  char(12), -- telephone
     Email char(30) check (Email like '%@%'), -- email
     fechaAlta date not null, -- date of registration
     fechaBaja date null, -- date of drop out
...
CREATE TABLE alquileres (
     id_alquiler INTEGER NOT NULL primary key, -- rental id
     nro_socio INTEGER NOT NULL, -- number of code of the member
     id_pelicula INTEGER NOT NULL, -- film id
     nro_copia INTEGER NOT NULL, -- copy number or code
     fecha_alquiler DATETIME NOT NULL, -- rental date
     fecha_devolucion dateTIME, -- date for returning (giving up) the rental
